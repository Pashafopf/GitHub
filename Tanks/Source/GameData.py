...
# GameData : этот модуль содержит все глобальные переменные и константы
screenrect     = Rect(0, 0, 1024, 768)      # константа: complete window
arenarect      = Rect(0, 0, 800, 768)       # константа: part of window to play in
scorerect      = Rect(800, 0, 224, 768)     # константа: part of the window with score board
tankspeed      = 1                          # константа: actual speed = tankspeed * tank.gear
angle          = 7.5                        # константа: if 90/angle is integer, the up, down, left & right movements will be straight
animstep = 0                                # переменная: отвечает за цикличность анимации

# Game Options
gearcooldown = 5                            # переменная: время между переключением передачи и задержка при изменении значения в окне опций
maxbullets = 5                              # переменная: максимальное число пуль на экране, или как сильно может нагреться пушка
bulletloadtime = 15                         # переменная: время перезарядки
bulletspeed = 8                             # переменная: скорость пуль
triggerhappiness = 3                        # переменная: сложность игры


#Прорисовка препятствий к каждому полю и добавление поля в каждый BattleGround
...
battlegroundnr = len(battleground)-1      # будем каждый раз начинать игру с предыдущего законченного поля битвы
...
mazes = [[Rect(0, 0, 1024, 1), Rect(0, 0, 1, 768), Rect(795, 0, 8, 768), Rect(0, 763, 1024, 1), Rect(1023, 0, 1, 768),
        ...
        []] # The Void                                                                      # переменная
mazenames = ["Ground Zero","Around The Blocks","The Monolith","The Void"]                   # константа
mazenumber = 0                                                                              # переменная
maze = mazes[mazenumber]                                                                    # переменная
mazename = mazenames[mazenumber]                                                            # переменная"""

# Координата верхнего левого угла и угол под котором будет поставлен гаражик: [x,y,angle]
respawnpoints = [[5,7,0], [745,717,180],[5,717,0],[745,7,180],[380,10,270],[380,717,90]]    # константный список

respawnlist = []    #переменный лист, в котором содержится информация, где танк последний раз появился,
                    # чтобы предотвратить следующее появление там же

# Цвета игроков и номера
red    = 0                                                                                  # константа
blue   = 1                                                                                  # константа
green  = 2                                                                                  # константа
yellow = 3                                                                                  # константа
grey   = 4                                                                                  # константа
purple = 5                                                                                  # константа
colour = ['red','blue','green','yellow','grey','purple']                                    # константа

gamestate = "splash screen"                                                                 #переменная
